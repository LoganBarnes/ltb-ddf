...
ArcBall::ArcBall(
    const Vector3& eye, const Vector3& viewCenter, const Vector3& upDir, Deg fov, const Vector2i& windowSize)
...
void ArcBall::translateDelta(const Vector2& translationNDC) {
    ...
    _targetPosition += _inverseView.transformVector({translationNDC.x() * hw, translationNDC.y() * hh, 0.0f});
}
